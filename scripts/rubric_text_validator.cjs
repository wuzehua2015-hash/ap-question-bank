...
function rubricQualityFindings(file, record) {
  ...
  if (subject === 'computer-science-a') {
    const rubric = record.rubric || {}
    const outline = String(rubric.solution_outline || '').replace(/\s+/g, ' ').trim()
    const reference = String(rubric.reference_solution || '').trim()
    const pointText = Array.isArray(rubric.points)
      ? rubric.points.map(point => point.description || point.criteria || point.text || '').join(' ')
      : ''

    if (outline.length < 120) {
      result.push({
        file: relative,
        question_id: record.question_id || record.id || 'unknown',
        field: 'rubric.solution_outline',
        rule: 'missing CSA solution outline',
        message: 'CSA FRQ rubric must include a student-usable solution outline before point rows.',
        sample: outline,
      })
    }
    ...
    if (!/```java[\s\S]{40,}?```/i.test(reference)) {
      result.push({
        file: relative,
        question_id: record.question_id || record.id || 'unknown',
        field: 'rubric.reference_solution',
        rule: 'missing CSA reference implementation',
        message: 'CSA FRQ rubric must include a fenced Java reference implementation or equivalent code skeleton.',
        sample: reference.slice(0, 260),
      })
    }

    if ((rubric.total_points || 0) >= 9 && (!Array.isArray(rubric.points) || rubric.points.length < 2 || pointText.length < 120)) {
      result.push({
        file: relative,
        question_id: record.question_id || record.id || 'unknown',
        field: 'rubric.points',
        rule: 'thin CSA scoring rows',
        message: 'CSA FRQ rubric must expose part-level scoring rows in addition to the reference implementation.',
        sample: pointText.slice(0, 260),
      })
    }

    for (const [field, value] of [
      ['rubric.solution_outline', outline],
      ['rubric.reference_solution', reference],
      ['rubric.points', pointText],
    ]) {
      for (const rule of RULES) {
        ...
        result.push({
          ...
          sample: String(value || '').slice(Math.max(0, match.index - 120), match.index + 260).replace(/\s+/g, ' '),
        })
      }
    }
  }

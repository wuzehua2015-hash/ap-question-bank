...
8. **表格数据跨页污染选项**（2015_Q60, 2017_Q54）：表格跨页时，表头数据泄漏到前一题的选项中，形成多行污染（如 `5\n2.` 或 `10\nPrice\nQuantity`）
9. **题号合并错误**（Q2/Q52 duplication）：正则 `\d+\s+\d+\.` 误将换行分隔的表格数据识别为拆分的题号（如 `5\n2.` → `52.`），导致题号跳跃和题目合并
10. **题号缺失**（Q47 missing）：同上，表格数据 `5\n2.` 被错误合并为 `52.`，导致 Q47 被跳过
11. **文本 span 断裂**（2015_Q06）：PDF 文本提取时，一个选项的文本被拆分为多个 y 坐标不同的 span（如 `(B) MU_s` 和 `/P_s > MU_h/P_h` 分在不同行），导致选项文本丢失或混乱
12. **图形坐标选项**（2013_Q43）：选项是图形上的坐标组合（如 Q₁,P₄），不是文本，无法通过文本提取获得，需要手动重建或标记为图形题
13. **图形标签误读为选项**（2017_Q37）：图形中的坐标标签（如 `B = Q₂`）被误读为选项文本，导致选项 B 的文本变为 `Q₂` 而丢失原意
```

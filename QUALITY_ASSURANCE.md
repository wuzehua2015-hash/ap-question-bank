...
Mock PDF 专项要求：
- [ ] 使用真实浏览器点击“下载 PDF”，不能只看页面截图或构建通过。
- [ ] 下载文件大小正常，不能是异常小文件或空白 PDF。
- [ ] 用 PyMuPDF 渲染全页缩略图，检查 MCQ、FRQ、答案页、FRQ 评分参考的分页边界。
- [ ] FRQ 题目必须各自从新页开始；长 FRQ 可以延续到下一页，但不得把另一道 FRQ 挤在同一页。
- [ ] 每道 FRQ 评分参考必须从新页开始。
- [ ] 修改 Mock PDF 分页时优先使用分段导出，不在长 DOM 内嵌套多层分页 class 或分页 style。
- [ ] Mock PDF 必须在 GitHub Pages 线上页面真实点击“下载 PDF”验证；本地页面、本地构建、页面截图都不能替代线上下载验收。
- [ ] 线上下载后必须记录 bundle 文件名、PDF 文件名、文件大小、页数、空白页检查结果和 PyMuPDF 缩略图路径。
- [ ] 分段导出必须有可观测进度；单个题块渲染不得让整份 PDF 无限停在“生成中”。
- [ ] 表格选项、代码块、长 FRQ、图片题是 PDF 导出的高风险段；如果普通 DOM 渲染超时，必须有内容不丢失的兜底路径。

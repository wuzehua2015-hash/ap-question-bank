...
学生端权限必须按三层设计，不得把内部管理状态暴露给学生：

| 层级 | 学生端名称 | 可用功能 |
|------|------------|----------|
| Tier 1 | 游客 | 单元 Quiz 在线训练；本机 localStorage 记录 |
| Tier 2 | 注册会员 | 单元 Quiz 在线训练、Mock Exam 在线训练、错题本、学习记录、账号进度同步 |
| Tier 3 | 翎英学员 | 搜索、题单、相似题、Quiz/Mock/成绩单 PDF 下载、单元知识点解析、完整题库工具 |

硬规则：
- 错题本和学习记录是注册会员功能，不是翎英学员专属功能。
- 错题和做题记录是教研、课程迭代、学生画像和未来教师端分析的数据基础设施；登录后必须能同步。
- 搜索、题单、相似题、PDF 下载、单元知识点解析仍属于翎英学员功能。
- 学生 UI 只使用 `注册会员` / `翎英学员`，不得显示 internal、certified、内部学生、已认证等内部口径。
- 新增“知识点解析”或类似内容讲解模块时，必须默认接入 Tier 3 gate；Tier 2 只能看到训练记录，不能看到单元知识点解析。
- `npm run validate` 必须运行 `scripts/access_contract_audit.cjs`，自动检查错题本/学习记录不能被 PremiumGate 挡住，搜索/PDF/相似题等高级能力不能绕过翎英学员 gate。
...
- 不能只依赖单一旧卷扩容。每个低容量科目都必须先做网络来源盘点，优先官方/AP Central/当前 CED/公开可靠来源；优先使用 2009 年及之后的来源，并把最新课程结构的材料放在最高优先级。
- 网络来源只能在通过来源可靠性、时效性、答案/评分完整性、题目结构质量、发布权限和去重检查后进入 source pack。第三方站点只可作为发现线索或重复校验，不能因为能访问就直接发布。
...
## 2026-07-13 学科管理与切换状态机

学生端必须把 `mySubjects` 当作唯一学习范围：

- 首页和顶部切换器只展示 `mySubjects`。
- 完整学科目录只放在 `/settings`。
- 首次访问可以没有已选科目，但 `/quiz`、`/exam`、`/search`、`/mistakes`、`/history` 等依赖当前科目的页面必须先显示选择科目入口，不能静默进入某个默认学科。
- 添加学科后立即设为当前学科和默认学科。
- 学生一旦有已选学科，界面层不能移除最后一个学科。
- 移除当前学科时，必须同步修正 `currentSubject` 和 `defaultSubject` 到剩余已选学科，不能留下悬空状态。
- Header、Home、Settings、直接 URL 进入页面必须共享同一套状态规则，不允许某个入口单独处理。

每次修改学科选择、账号同步设置、首页学科入口、Header 导航或新增依赖当前学科的页面，都必须执行以下浏览器路径：

- 清空 `mySubjects/currentSubject/defaultSubject` 后直接访问 `/quiz`，应显示选择科目入口。
- 从 Settings 添加一个学科，应成为当前/默认学科。
- 只有一个学科时，顶部切换器仍可打开并显示管理入口。
- 尝试移除最后一个学科，应提示并保留原状态。
- 添加第二个学科后移除当前学科，应回落到剩余学科。
- 回到首页和进入 `/quiz`，应使用回落后的当前学科。
...
## Student Web Layout Rules (2026-07-13)

- Maintain the home page as a learning dashboard: current subject, primary practice action, mock exam action, account state, selected subjects, and common tools need clear hierarchy.
- Keep top navigation lightweight, stable, and Chinese-first. The subject switcher should show only the student's selected subjects.
- Student-facing copy should describe direct student value. Teaching research, data infrastructure, and system-iteration rationale belong in maintenance records, not the student UI.
- Any change to the home page, header, footer, login/register flow, or account page requires real-browser desktop and mobile checks for text overlap, button crowding, unbalanced empty space, and wrong access-tier labels.
